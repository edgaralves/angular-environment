...
return this.data.vars[this.get()];};this.is=function(environment){return(environment===this.environment);};this.check=function(){var	local=window.location.host,self=this;angular.forEach(this.data.domains,function(v,k){angular.forEach(v,function(v){if(local.match(v.replace("*", "(.*)"))){self.environment=k;}});});};this.$get=function(){return this;};});